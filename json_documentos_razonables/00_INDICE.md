# JSON razonables por tipo de documento

Estos archivos son ejemplos **de negocio** basados en los XSD que subiste.

Criterios usados:
- se incluyen campos razonables y frecuentes
- no se incluye `TED` ni `Signature`
- en documentos afectos se agregó una variante `solo_afecto` y otra `mixto_afecto_exento` cuando aplica
- los valores son de ejemplo y deben reemplazarse por datos reales

## Archivos
- `BOLETA_39_mixto_afecto_exento.json`
- `BOLETA_39_solo_afecto.json`
- `BOLETA_41_Exenta.json`
- `DTE_110_Factura_de_Exportacion_Electronica.json`
- `DTE_111_Nota_de_Debito_de_Exportacion_Electronica.json`
- `DTE_112_Nota_de_Credito_de_Exportacion_Electronica.json`
- `DTE_33_Factura_Electronica_mixto_afecto_exento.json`
- `DTE_33_Factura_Electronica_solo_afecto.json`
- `DTE_34_Factura_Electronica_Exenta.json`
- `DTE_43_Liquidacion_Factura_Electronica_mixto_afecto_exento.json`
- `DTE_43_Liquidacion_Factura_Electronica_solo_afecto.json`
- `DTE_46_Factura_de_Compra_Electronica_mixto_afecto_exento.json`
- `DTE_46_Factura_de_Compra_Electronica_solo_afecto.json`
- `DTE_52_Guia_de_Despacho_Electronica_mixto_afecto_exento.json`
- `DTE_52_Guia_de_Despacho_Electronica_solo_afecto.json`
- `DTE_56_Nota_de_Debito_Electronica_mixto_afecto_exento.json`
- `DTE_56_Nota_de_Debito_Electronica_solo_afecto.json`
- `DTE_61_Nota_de_Credito_Electronica_mixto_afecto_exento.json`
- `DTE_61_Nota_de_Credito_Electronica_solo_afecto.json`
